...
type TimingBreakdownEdges struct {
	// ExectionInfo holds the value of the exection_info edge.
	ExectionInfo []*ExectionInfo `json:"exection_info,omitempty"`
	...
	namedExectionInfo map[string][]*ExectionInfo
	namedChild        map[string][]*TimingChild
}

// ExectionInfoOrErr returns the ExectionInfo value or an error if the edge
// was not loaded in eager-loading.
func (e TimingBreakdownEdges) ExectionInfoOrErr() ([]*ExectionInfo, error) {
	if e.loadedTypes[0] {
		return e.ExectionInfo, nil
	}
	return nil, &NotLoadedError{edge: "exection_info"}
}
...
// QueryExectionInfo queries the "exection_info" edge of the TimingBreakdown entity.
func (tb *TimingBreakdown) QueryExectionInfo() *ExectionInfoQuery {
	return NewTimingBreakdownClient(tb.config).QueryExectionInfo(tb)
}
...
// NamedExectionInfo returns the ExectionInfo named value or an error if the edge was not
// loaded in eager-loading with this name.
func (tb *TimingBreakdown) NamedExectionInfo(name string) ([]*ExectionInfo, error) {
	if tb.Edges.namedExectionInfo == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := tb.Edges.namedExectionInfo[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (tb *TimingBreakdown) appendNamedExectionInfo(name string, edges ...*ExectionInfo) {
	if tb.Edges.namedExectionInfo == nil {
		tb.Edges.namedExectionInfo = make(map[string][]*ExectionInfo)
	}
	if len(edges) == 0 {
		tb.Edges.namedExectionInfo[name] = []*ExectionInfo{}
	} else {
		tb.Edges.namedExectionInfo[name] = append(tb.Edges.namedExectionInfo[name], edges...)
	}
}
